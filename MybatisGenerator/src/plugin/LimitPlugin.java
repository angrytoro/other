...
public class LimitPlugin extends PluginAdapter {
	...
	public boolean providerSelectByExampleWithoutBLOBsMethodGenerated(
			...
			IntrospectedTable introspectedTable) {
		method.addBodyLine(method.getBodyLines().size() - 2,
				"if (example.getLimitStart() >= 0)");
		method.getBodyLines()
				.set(method.getBodyLines().size() - 1,
						"return SQL() + \" limit #{limitStart,jdbcType=INTEGER} , #{limitCount,jdbcType=INTEGER}\";");
		method.addBodyLine("else");
		method.addBodyLine("return SQL();");
		return super.providerSelectByExampleWithoutBLOBsMethodGenerated(method,
